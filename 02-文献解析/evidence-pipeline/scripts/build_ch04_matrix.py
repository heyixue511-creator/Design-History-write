...
STRUCT = json.load(open(ROOT / "logs" / "_ch04_structure.json", encoding="utf-8"))
...
csv_rows, md_lines = [], []
md_lines.append("# 第四章核心命题—来源矩阵（V2滚动版）\n")
md_lines.append("> 生成依据仅为人工复核映射。A—D／X表示章节角色，不表示来源质量；所有条目仍为V2，不能逐字引用。本章BATCH-005含15个来源（Werkbund/AEG、包豪斯五层、Isotype、Loos、维也纳工坊、风格派与东亚转译）；与第三章共享风格派对象，同一对象只计一份P0。\n")
...
with (OUT / "CH04_核心命题—来源矩阵.csv").open("w", encoding="utf-8-sig", newline="") as h:
    w = csv.DictWriter(h, fieldnames=fields)
    w.writeheader()
    w.writerows(csv_rows)
(OUT / "CH04_核心命题—来源矩阵.md").write_text("\n".join(md_lines) + "\n", encoding="utf-8")
