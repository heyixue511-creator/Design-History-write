...
STRUCT = json.load(open(ROOT / "logs" / "_ch02_structure.json", encoding="utf-8"))
...
csv_rows, md_lines = [], []
md_lines.append("# 第二章核心命题—来源矩阵（V2滚动版）\n")
md_lines.append("> 生成依据仅为人工复核映射。A—D／X表示章节角色，不表示来源质量；所有条目仍为V2，不能逐字引用。本章涉及B0101／B0102／B0478版本族与多个共享对象组，重复与共享P0规则见`10_证据缺口与审计`第二章收束文件。\n")
...
with (OUT / "CH02_核心命题—来源矩阵.csv").open("w", encoding="utf-8-sig", newline="") as h:
    w = csv.DictWriter(h, fieldnames=fields)
    w.writeheader()
    w.writerows(csv_rows)
(OUT / "CH02_核心命题—来源矩阵.md").write_text("\n".join(md_lines) + "\n", encoding="utf-8")
