#!/usr/bin/env python3
"""Aggregate all reviewed mappings for chapter 8 sections -> structure JSON for matrix authoring."""
import csv, json
    ...
    for m in sorted((batch_dir / "mappings").glob("*.csv")):
        with m.open(encoding="utf-8-sig", newline="") as h:
            for row in csv.DictReader(h):
                if row["section_id"].startswith("8."):
...
(ROOT / "logs" / "_ch08_structure.json").write_text(
    json.dumps(out, ensure_ascii=False, indent=1), encoding="utf-8")
print("WROTE", len(out), "sections:", {k: v["count"] for k, v in out.items()})
